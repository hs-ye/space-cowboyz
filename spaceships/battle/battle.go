...
func (f *Fleet) AttackRound(targetfleet *Fleet) {
	...
	for _, s := range f.Ships {
		...
		//TODO loop through different weapon systems to target/fire
		if s.Weapon().target == nil {
			s.Findtarget(targetfleet) // currently target set at ship level
		}
...
func (b *Battle) StartBattle() {
	...
	for timer = 0; f1.canfight && f2.canfight && timer < MaxBattleRounds; timer++ {
		...
		f1s = f1.CanFight()
		f2s = f2.CanFight()
		fmt.Printf("Ships remaining: Fleet1 %d - Fleet2 %d\n", f1s, f2s)
	}
